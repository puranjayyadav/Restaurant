...
def prepare_data(input_data: str) -> List[Any]:
    ...
    try:
        ...
        # Don't print error for every retry - only on final failure
...
def search_place_by_name(place_name: str, retries: int = 0, max_retries: int = 3) -> Optional[Dict[str, Any]]:
    ...
    Returns:
        Dictionary with lat, lon, and place info, or None if not found
    """
    try:
        # Use a generic location (center of US) to search for the place
        # We'll use the same scraping endpoint but with the place name as query
        ...
        if response.status_code == 200:
            ...
            # Remove /*""*/ at the end (matches Dart implementation)
            if len(html) >= 6 and html.endswith('/*""*/'):
                data = html[:len(html) - 6]
            else:
                data = html
            
            # Use the same prepare_data function that handles all formats
...
def get_google_maps_data(
    ...
    Args:
        query: Search query (e.g., "restaurant", "coffee shop") or None if searching by place_name
        lat: Latitude (optional if place_name is provided)
        lon: Longitude (optional if place_name is provided)
        place_name: Name of a place to search around (e.g., "BKK New York")
        zoom: Zoom level (default: 13499.795714815926)
        count: Number of results to fetch (default: 200)
    ...
    try:
        ...
        if place_name and (lat is None or lon is None):
            ...
            lat = place_info['lat']
            lon = place_info['lon']
            print(f"Found {place_info['name']} at {lat}, {lon}")
            # If no query provided, use a generic search around the place
            if not query:
                query = "restaurant"
        ...
        if response.status_code == 200:
            ...
            if html.strip().startswith('<!') or '<html' in html.lower():
                print(f"ERROR: Received HTML response instead of JSON. Status: {response.status_code}")
                if retries < max_retries:
                    time.sleep(2)
            ...
            print(f"ERROR: Google Maps scraping returned {response.status_code}")
            if retries < max_retries:
                time.sleep(2)
...
def get_long_lat_grid(place: Dict[str, Any], desired_grid_length: int = 5) -> List[Tuple[float, float]]:
    """
    Generates a grid of lat/long coordinates from a place's viewport.
    Useful for searching large areas by breaking them into smaller grids.
    
    Args:
        place: Place dictionary with geometry.viewport structure
        desired_grid_length: Number of grid points per dimension
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("Method 1: Search by place name")
    print("=" * 60)
    place_name = "BKK New York"
    print(f"\nSearching for places near '{place_name}'...")
    results = get_google_maps_data(
        query="restaurant",  # Search for restaurants near this place
    ...
    for i, place in enumerate(results[:5], 1):
        ...
        print(f"   Address: {place.get('full_address', 'N/A')}")
        print(f"   Rating: {place.get('avg_rating', 'N/A')} ({place.get('total_reviews', 0)} reviews)")
        print(f"   Photos: {len(place.get('photos', []))}")
    
    # Method 2: Search by coordinates
    print("\n" + "=" * 60)
    print("Method 2: Search by coordinates")
    print("=" * 60)
    query = "restaurant"
    lat = 40.7128
    lon = -74.0060
    
    print(f"\nSearching for '{query}' near {lat}, {lon}...")
    results = get_google_maps_data(query, lat, lon, count=20)
