...
async function scrapeReviewsByPlaceId(placeId, maxReviews = 5) {
    ...
    try {
        ...
        // REFRESH TO LOAD REVIEWS (user tip!)
...
if (isMainModule) {
    ...
    console.log(`\n${'='.repeat(60)}`);
    console.log(`  Google Maps Review Scraper - WITH REFRESH`);
    console.log(`${'='.repeat(60)}`);

    if (!placeId) {
        console.log('\n❌ Error: Please provide a place_id');
        console.log('\nUsage: node working_scraper.js <place_id> [max_reviews]');
        console.log('Example: node working_scraper.js "ChIJpZd5uFtbwokROm9FtRhLhyQ" 5\n');
        process.exit(1);
    }
